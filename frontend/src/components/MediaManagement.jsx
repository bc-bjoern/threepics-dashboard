...
export default function MediaManagement() {
  ...
  if (loading) return <p>Lade Medien...</p>;
  if (error) return <p style={{ color: 'red' }}>{error}</p>;

  if (mediaList.length === 0) return <p>Keine Medien zum Anzeigen.</p>;
          ...
          >
            {type === 'image' && (
              <img
                src={`${backendUrl}${url}`}
                alt={subtitle || 'Bild'}
                style={{ height: 60, marginRight: 16, objectFit: 'cover' }}
              />
            )}
            {type === 'video' && (
              <video
